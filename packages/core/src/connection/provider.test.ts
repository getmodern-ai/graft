 ...
 * provider is played by a fake shaped like GRA-58's and GRA-59's, since neither exists yet.
 */
...
/** A relay provider covering one vendor, connecting with a link — GRA-59's shape. */
function linkProvider(name = "broker", covers = (vendor: string) => vendor === "gmail") {
...
describe("the keyring provider", () => {
  it("is today's behaviour: covers every vendor, connects through the form over every signing scheme, releases nothing", async () => {
    ...
    expect(keyringProvider.covers("anything", ["api.anything.example"])).toBe(true);
...
describe("the provider list", () => {
  ...
  it("routes a proposal to the first provider that covers it, the keyring catching the rest", () => {
    const { provider } = linkProvider();
    ...
    expect(providerFor(DEFAULT_PROVIDERS, "gmail", ["gmail.googleapis.com"])).toBe(keyringProvider);
  });

  it("throws for a list nothing in which covers the vendor — a list assembled without the keyring", () => {
    const { provider } = linkProvider();
    expect(() => providerFor([provider], "unleashed", [])).toThrow(/keyring should always be last/);
  });
