 ...
 * seven-harness picker's (`graft-marketing`'s `src/lib/setup-prompt.ts`), so a person who arrives
 * from "Try for free" recognises the list. `agentName` is what the agent is called when the person
 * does not name it under *Advanced options*.
 */
...
export const SETUP_HARNESSES = [
  ...
  { id: "codex", label: "Codex", description: "App, CLI & IDE", kind: "oauth", agentName: "Codex" },
  { id: "hermes", label: "Hermes", description: "Local agent", kind: "token", agentName: "Hermes" },
  {
    id: "openclaw",
    label: "OpenClaw",
