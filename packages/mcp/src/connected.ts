 ...
 * it reconnected (Greptile on #88). The core holds no notifier, so this lives here and every path
 * that creates or reconnects a row calls it after its transaction: `request_connection`'s
 * gateway connect and connection-ask settle (`connection-request.ts`), the ask card's keyless
 * confirm (`tools/answer-ask.ts`), and the server's create, reconnect, credential re-entry, ask
 * submit and link return (`apps/server/src/api.ts`, `provider-link.ts`). Answers the agents told,
 * sorted, so a caller's answer can name them as the revoke's does.
 */
