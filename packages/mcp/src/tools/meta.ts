 ...
 * `find_tool`, `promote`, `demote`, `run_tool` (ADR 0003, ADR 0009) — the two connection handoffs,
 * `request_connection` and `request_credential` (GRA-28; `../connection-request.ts`), and the two
 * whose ticket has not landed, present from day one so the list is stable and answering
 * `not_available_yet` with the ticket: `acquire` and `acquire_status` (GRA-29). The advanced set is
 * `authoring.ts`; the per-connection execute tools are `execute.ts`.
 */
