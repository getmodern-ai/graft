 ...
 * (ADR 0003, ADR 0006, ADR 0008 as amended) and whose tool `answer_ask` is. The tool descriptions
...
export const SERVER_INSTRUCTIONS = [
  "Your working set from Graft: the authored tools promoted for you, callable as vendor__name, and these fixed ones.",
  "When a task has no tool, work in this order. Call find_tool first: a demoted match may exist. If one does, promote it: in your list at once, no authoring needed. If the vendor has no connection in your scope, call request_connection. Call acquire only when nothing fits. It answers a jobId at once: poll acquire_status and relay the newest progress line in one sentence, only when it changed. Do not start a second acquire for the same goal. There is no route to a vendor except through a Graft tool. Do not drive the authoring tools (read_web_page, write_file, check_tool, publish_tool) or an execute__ tool unless the person asked you to author by hand.",
  "Any answer with a url and an awaiting_ word (approval, connection, credential, scope) is a handoff: the next step is the person's, in the console. Send them the link exactly as returned, then wait; when they say it is done, call the same tool again with the same arguments. Never ask the person for an API key, a password or a token in chat, whatever the vendor calls it. The console is where secrets go; you never see one. Never propose a made-up key for a vendor that documents none. A rotated or expired credential is request_credential on the existing connection, never a new one. The connection page offers the build approval, on by default; left on, acquire starts without a second link, so do not tell the person to expect one.",
  "Some clients snapshot the tool list per conversation, so a new tool may be missing: run_tool { vendor, name, input } calls it by name. The acquire result and find_tool carry the tool's inputSchema for run_tool. Re-fetch on notifications/tools/list_changed.",
  "Approvals are the person's. A read-only tool never asks. Any other tool asks once, and the answer holds, a destructive tool too; the person can set a tool to ask every time in the console. acquire asks once per agent per connection. answer_ask is the ask card's, not yours.",
].join("\n\n");
