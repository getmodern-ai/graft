...
export const SERVER_INSTRUCTIONS = [
  ...
  "Any answer with a url and an awaiting_ word (approval, connection, credential, scope) is a handoff: the next step is the person's, in the console. Send them the link exactly as returned, say what it is for, then wait; when they say it is done, call the same tool again with the same arguments. Never ask the person for an API key, a password or a token in chat, whatever the vendor calls it. The console is where secrets go; you never see one.",
