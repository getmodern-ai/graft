...
reset from *Forgot password?*: with `GRAFT_LOOPS_API_KEY` set the link is emailed through Loops,
without it the link is printed in `docker compose logs graft` (ADR 0021). Then The token is shown once; put it where your harness reads environment
