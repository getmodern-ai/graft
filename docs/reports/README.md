...
| [GRA-114](https://linear.app/get-modern/issue/GRA-114) | [#92](https://github.com/getmodern-ai/graft/pull/92) | — | every meta-tool declares both hints outright: `destructiveHint: false` on every fixed tool but `run_tool` and `execute__<id>`, which keep `true` with one sentence saying the hint is the carried request's, since MCP reads an unset hint as true and ChatGPT's Actions list tagged six tools DESTRUCTIVE for it; `session.test.ts` pins both hints per tool |
