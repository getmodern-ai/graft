...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-scrim text-xs/relaxed transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
...
function SheetContent({
  ...
  /**
   * Styles the backdrop, which `className` cannot reach — it is a sibling this renders above the
   * popup, not a child. The scrim blurs as well as dims, so a sheet sharing the screen with
   * chrome that stays readable (the sidebar sheet beside the agent rail, CAN-326) offsets the
   * scrim to where the sheet starts rather than letting it fog the chrome.
   */
