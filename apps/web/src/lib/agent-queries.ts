...
export type ConnectedHarness = Jsonified<AgentDetailOutput>["connectedHarnesses"][number];
...
export const agentQuery = (agentId: string) =>
  queryOptions({
    ...
    queryFn: () => api<Jsonified<AgentDetailOutput>>(`/agents/${encodeURIComponent(agentId)}`),
  });
