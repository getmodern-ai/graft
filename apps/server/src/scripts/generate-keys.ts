...
/**
 * A fresh Ed25519 key pair and a keyring secret, printed as `.env` lines:
 *
 *   pnpm --filter @graft/server keys >> apps/server/.env
 *
 * The PEMs are `\n`-escaped inside double quotes, which is the one spelling a `.env` file can hold
 * and one `@graft/env` accepts. The secret is 32 URL-safe characters — the minimum
 * `GRAFT_KEYRING_SECRET` allows. For a deployment, generate the pair with `openssl genpkey
