...
rotate with a thirty-second reuse window and live with the agent; revoking the agent revokes every
token in the same transaction. No variable is added: the issuer is `GRAFT_AUTH_URL`'s origin and the
