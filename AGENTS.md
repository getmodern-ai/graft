...
(`@better-auth/cli` stopped at 1.4.21), pinned in that script to the catalog's `better-auth` version
so the generator and the runtime agree on the schema. Regenerate rather than edit when the auth
configuration or that version changes, then `db:generate`: Better Auth 1.7.3 validates the schema
at init and refuses a required column it never writes (GRA-86, migration 0008).
